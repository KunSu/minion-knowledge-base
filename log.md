...
2026-08-14 | remember | wiki/knowledge/boa-statement-download.md, wiki/knowledge/chase-statement-download.md, index.md | 新增 BoA 对账单批量下载页 + Chase 页加交叉链接。实测 4 个 BoA 账户 104 份(4921 存款 27、4109 信用卡 26、1277 房贷 51)。BoA 是两步链路(DOM 的 data-docid → docViewDownload),比 Chase 少一步,但四个坑更险:①Chrome 对未授权域每次脚本执行只放行 1 个下载(blob 与 window.open 皆然)——报告 25 份成功实际只落地 1 份;Chase 能连下 167 个是因为该域此前已被授予权限,不可推广。解法是页内 fetch 全部→手写 store-only ZIP→只触发 1 次下载。②曾试图把实时会话凭证(SSOTOKEN/SMSESSION/ah_token)落盘用 curl 绕过,被权限层拦下且拦得对,凭证必须留在浏览器会话内。③docViewDownload 认页面当前选中年份,下拉停在 2024 时 2025/2026 的 id 一律返回 76100 字节 HTML(HTTP 200 不报错),必须先切年份再抓。④折叠面板名按账户类型变化,房贷是 Statements and Escrow Analysis,用 /^Statements$/ 精确匹配导致 1277 除当年外全报 0 份——看起来完全像银行没数据,改前缀匹配后拿到 51 份;由此立规:任何 0 份结果必须读面板实际文字确认是银行说没有还是选择器没点开。另记 owner 不能从对账单姓名栏推断——信用卡表头只列 primary cardholder,我据此把 joint 的 4109 误判为个人账户,已由 Owner 纠正。收尾核对改用月份连续性(区间内逐月找断月)而非数量,271 份零断月
