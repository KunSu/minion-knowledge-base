...
2026-08-14 实测跑通:一次性下载 3 个存款/信用卡账户 × 2022–2026 共 167 份对账单,0 失败。
...
- **能不能访问已开的银行 tab,先调一次工具再答** —— 凭印象回答「不行」是错的。
...
**实测验证到 `_4`(即回溯 5 个年份)。** UI 的「View:」下拉列出 8 个年份选项,推断上限是 `CURRENT_YEAR_MINUS_6`(7 年,与页面自述的「7 years of statements」一致),但**更早的档位未实测**。
...
const CSRF    = '<会话级 csrftoken,见下节「拿 csrftoken」>';
// ───────────────────────────────────────────────
...
for (const [yr, F] of [['本年','CURRENT_YEAR'],['上一年','CURRENT_YEAR_MINUS_1']]) {
  ...
  for (const d of refs) {
    ...
    if (blob.size < 2000) continue;                        // 失败常表现为几 KB 的 JSON 错误页
...
**不要相信脚本自己的成功计数器** —— 计数器统计的是「请求成功 + 触发点击」,不是文件落地。收尾必须回到文件系统。
